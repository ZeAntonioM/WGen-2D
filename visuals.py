...
# --- 1. WIND ENGINE (Logic from previous steps) ---
class WindEngine:
    ...
    def get_wind_at(self, x, y):
        # Sample noise to get a vector (nx, ny)
...
def draw_infinite_grid(surface, camera_pos):
    """
    Draws grid lines that align with the CHUNK boundaries.
    The lines scroll with the world.
    """
    screen_w, screen_h = surface.get_size()
    
    # 1. Calculate the Top-Left coordinate of the visible world
    view_left = int(camera_pos.x - screen_w // 2)
    view_top = int(camera_pos.y - screen_h // 2)
    
    # 2. Determine grid spacing (Use your actual Chunk Size)
    # Ensuring we use int for pixel coordinates
    step_x = int(CHUNK_SIZE.x)
    step_y = int(CHUNK_SIZE.y)
    
    # 3. Find the first grid line that should be visible
    # (Snap view_left to the nearest multiple of chunk size)
...
def draw_wind_arrows(surface, camera_pos, grid_spacing=64):
    ...
    # Snap start pos so arrows don't jitter
...
# Player/Camera starts at 0,0 (or anywhere you want)
